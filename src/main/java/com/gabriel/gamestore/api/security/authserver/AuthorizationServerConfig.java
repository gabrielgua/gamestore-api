package com.gabriel.gamestore.api.security.authserver;
...
public class AuthorizationServerConfig {
    ...
    @Bean
    public RegisteredClientRepository repository(PasswordEncoder passwordEncoder, AuthProperties properties) {
        var angularClient = RegisteredClient
                ...
                .redirectUris(uris -> uris.addAll(properties.getAllowedRedirects()))
                .scope("READ")
                .scope("WRITE")
    ...
    public OAuth2TokenCustomizer<JwtEncodingContext> jwtEncodingContextOAuth2TokenCustomizer(UsuarioRepository usuarioRepository) {
        return (context -> {
            ...
            if (authentication.getPrincipal() instanceof User user) {
                var usuario = usuarioRepository.findByUsername(user.getUsername()).orElseThrow();
                ...
                context.getClaims().claim("userId", usuario.getId().toString());
    ...
    public JWKSet jwkSet(AuthProperties properties) throws Exception {
        ...
        var rsaKey = RSAKey.load(keyStore,
